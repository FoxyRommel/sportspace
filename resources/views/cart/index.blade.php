...
@section('content')
    ...
    <section>
        <!-- Section heading -->
        <h1 class="mb-8 text-lg lg:text-[36px] font-black text-black">Корзина покупок</h1>
        ...
        @else
            <!-- Message -->
            <div class="lg:hidden py-3 px-6 rounded-lg bg-blue text-white">Таблицу можно пролистать вправо →</div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
                <div class="text-[32px] font-black text-black">Итого: {{ cart()->amount() }} </div>
                <div class="pb-3 lg:pb-0">
                    <form action="{{ route('cart.truncate') }}" method="POST">
                        @csrf
                        @method('DELETE')

                        <button type="submit" class="btn btn-purple font-medium">Очистить корзину</button>
                    </form>
                </div>
